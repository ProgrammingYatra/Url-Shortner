...
const createUrl = async function (req, res) {
  try {
    ...
    if (!isVAlidRequestBody(body))
      return res
        .status(400)
        .send({ status: false, message: "Please provide data to create Url" });
    ...
    if (!isValid(longUrl))
      return res
        ...
        .send({
          status: false,
          message: "The longUrl is mandatory and Should have non empty string",
        });
    ...
    if (cachedUrlData) {
      ...
      return res
        ...
        .send({
          status: false,
          message: `This Url is already in Cache so use this shortUrl ${cacheShortUrl.shortUrl}`,
        });
    }
    ...
    if (!urlRegex.test(longUrl))
      return res
        ...
        .send({
          status: false,
          message: "Please give the long url in valid Formate",
        });
    ...
    if (getlongUrl) {
      ...
      return res
        ...
        .send({
          status: false,
          message: `For This LongUrl use this ShortUrl ${getlongUrl.shortUrl} which is already Registered in DB`,
        });
    }

    let baseUrl = "http://localhost:3000";

    if (!baseUrlRegex.test(baseUrl))
      return res
        .status(400)
        .send({
          status: false,
          message: "Please give the baseurl in valid Formate",
        });
  ...
  try {
    ...
    if (!findUrl)
      return res
        ...
        .send({
          status: false,
          message: `This ${req.params.urlCode} urlCode is not found.`,
        });
